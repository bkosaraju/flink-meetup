...
//      val comparableRules = ruleEvent.filterNot(itm => META_COLUMNS.contains(itm._1))
...
//      if (inputColumns.intersect(ruleKeys).length.equals(ruleKeys.length) && ! actionRow.getFieldNames(true).contains(RULE_NAME)) {
...
//          actionRow.setField(RULE_NAME, ruleId)
//        }
//      }
//    }
//    )
...
//
//    if (! actionRow.getFieldNames(true).contains(RULE_NAME)) {
//      actionRow.setField(RULE_NAME, "-1")
//      actionRow.setField(ACTION_POLICY_ID, "-1")
